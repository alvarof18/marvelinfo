    ...
    companion object {
        fun marvelResponseToComicEntity(marvelResponse: Resultado): ComicsModel {
            return ComicsModel(
                ...
                urlPoster = "${marvelResponse.thumbnail.path.replace("http","https")}/portrait_uncanny.${marvelResponse.thumbnail.extension}",
                creators = marvelResponse.creators.items.map { item -> CreatorsModel(name = item.name, role = item.role)
                },
